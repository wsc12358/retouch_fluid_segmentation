...
# 生成渐变色
def gradient_rgb_color(color_list, color_sum=100):
    color_center_count = len(color_list)
    # if color_center_count == 2:
    #     color_center_count = 1
    color_sub_count = int(color_sum / (color_center_count - 1))
    ...
    for color_index_end in range(1, color_center_count):
        ...
        for color_index in range(1, color_sub_count):
            now_color = [int(now_color[0] + r_step), int(now_color[1] + g_step), int(now_color[2] + b_step)]
            color_map.append(now_color)
        color_index_start = color_index_end
    return color_map
...
if __name__ == '__main__':
    ...
    input_colors=["#02007D", "#01CDFF", "FEEA02", "#840000"]
    colors = gradient_color(input_colors,color_sum=1000)
    sns.palplot(colors)
    print(colors)
    print(len(colors))
    plt.show()
